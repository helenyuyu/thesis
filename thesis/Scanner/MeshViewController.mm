...
- (void)viewDidLoad
{
    ...
    //_categories = @[@"Bed", @"Chair", @"Couch", @"Drawers", @"Table", @"TV", @"Other"];
...
- (IBAction)saveModel:(id) sender {
    UIAlertController *alertController = [UIAlertController
                                          alertControllerWithTitle:@"Save model"
                                          message:nil
    ...
    [alertController addTextFieldWithConfigurationHandler:^(UITextField *textField)
     {
         textField.clearButtonMode = UITextFieldViewModeWhileEditing;
         textField.placeholder = @"Category (e.g. chair)";
     }];
    ...
    UIAlertAction *okAction = [UIAlertAction
                               ...
                               handler:^(UIAlertAction *action)
                               {
                                   ...
                                   NSString *category = alertController.textFields[1].text;
                                   NSString* dir = [NSSearchPathForDirectoriesInDomains( NSDocumentDirectory, NSUserDomainMask, YES ) objectAtIndex:0];
                                   NSString* uuid = [[NSUUID new] UUIDString];
                                   NSString* screenshotFilename = [NSString stringWithFormat: @"Screenshot%@.jpg", uuid];
                                   NSString* meshFilename = [NSString stringWithFormat: @"Mesh%@.obj", uuid];
                                   NSString *screenshotPath =[dir stringByAppendingPathComponent:screenshotFilename];
                                   NSString *meshPath = [dir stringByAppendingPathComponent:meshFilename];
                                   ...
                                   NSError* error;
                                   BOOL success = [meshToSend writeToFile:meshPath options:options error:&error];
                                   if (!success)
                                   {
                                       ...
                                       UIAlertController* alert = [UIAlertController alertControllerWithTitle:@"The mesh could not be saved."
                                                                                                      message: [NSString stringWithFormat:@"Save failed: %@.",[error localizedDescription]]
                                                                                               preferredStyle:UIAlertControllerStyleAlert];
                                   ...
                                   ScanManager *scanManager = [(AppDelegate *)[[UIApplication sharedApplication] delegate] scanManager];
                                   [scanManager createScanWithMeshFilename: meshFilename
                                                     andScreenshotFilename: screenshotFilename
                                                              andTitle: title
